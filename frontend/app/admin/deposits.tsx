...
export default function AdminDeposits() {
  ...
  const load = useCallback(async () => { try { setItems(await api.adminDeposits(status)); } catch {} }, [status]);
  useFocusEffect(useCallback(() => { load(); }, [load]));

  const act = async (id: string, action: 'approve' | 'reject') => { try { await api.adminActDeposit(id, action); load(); } catch (e: any) { alert(e.message); } };

  return (
    <View style={{ flex: 1, backgroundColor: colors.surfaceSecondary }}>
      <SafeAreaView edges={['top']} style={s.header}>
        <Pressable onPress={() => router.back()}><Ionicons name="chevron-back" size={22} color={colors.onSurface} /></Pressable>
        <Text style={s.title}>Deposit Requests</Text>
        <View style={{ width: 22 }} />
      </SafeAreaView>
      <View style={s.segment}>
        {(['pending', 'approved', 'rejected'] as const).map(k => (
          <Pressable key={k} testID={`status-${k}`} onPress={() => setStatus(k)} style={[s.segBtn, status === k && s.segActive]}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.lg }}>
        {items.map(d => (
          <View key={d.id} style={s.card} testID={`deposit-${d.id}`}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <View>
                <Text style={s.name}>{d.user_name} · {d.user_phone}</Text>
                <Text style={s.sub}>UTR: {d.utr}</Text>
                <Text style={s.date}>{new Date(d.created_at).toLocaleString()}</Text>
              </View>
              <Text style={s.amt}>₹{d.amount.toFixed(0)}</Text>
            </View>
            {d.status === 'pending' && (
              <View style={s.row2}>
                <Pressable testID={`approve-${d.id}`} onPress={() => act(d.id, 'approve')} style={[s.btn, { backgroundColor: '#2ECA7F' }]}><Text style={s.btnT}>Approve</Text></Pressable>
                <Pressable testID={`reject-${d.id}`} onPress={() => act(d.id, 'reject')} style={[s.btn, { backgroundColor: '#FF4D4F' }]}><Text style={s.btnT}>Reject</Text></Pressable>
              </View>
            )}
          </View>
        ))}
        {items.length === 0 && <Text style={{ textAlign: 'center', color: colors.onSurfaceMuted, marginTop: 40 }}>No {status} deposits</Text>}
      </ScrollView>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  date: { fontSize: 11, color: colors.onSurfaceMuted, marginTop: 2 },
